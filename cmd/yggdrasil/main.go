...
func main() {
	// Set Main Operation
	const op = "ratatoskr.yggdrasil.api"
	
	// Set Binary Start Time
	StartTime = time.Now()
	
	// Default Config file based on the environment variable
	defaultConfigFile := "configs/yggdrasil/config-local.yaml"
	if env := os.Getenv("APP_MODE"); env != "" {
		defaultConfigFile = fmt.Sprintf("configs/yggdrasil/config-%s.yaml", env)
	}
	...
	// Connect to Stan
	clientId := fmt.Sprintf("ratatoskr_yggdrasil_%s", fmt.Sprintf("%x", md5.Sum([]byte(hostname))))
